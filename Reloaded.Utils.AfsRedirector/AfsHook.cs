...
{
    private readonly AfsFileTracker _afsFileTracker;

    private readonly AfsBuilderCollection _builderCollection = new();
    private readonly Dictionary<string, VirtualAfs> _virtualAfsFiles = new(StringComparer.OrdinalIgnoreCase);

    public AfsHook(NativeFunctions functions)
    {
        _afsFileTracker = new(functions);
